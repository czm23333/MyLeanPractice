...
theorem P9 : [Group T] → (∀ x : T, orderOf x ≠ 2) → (∀ x : T, ∀ y : T, (x * y)^2 = (y * x)^2) → ∀ x : T, ∀ y : T, x * y = y * x := by
  ...
  let tmp3 : a * a * b⁻¹ * a = a * b := by
    simp [← pow_two, commsq x y]
    rw [pow_two]
    simp [mul_assoc]
    rw [← mul_assoc x x y, ← pow_two, ← mul_assoc y y x, ← pow_two]
    rw [← mul_assoc]
    rw [← sqcomm x y, sqcomm y x]
    rw [← mul_assoc]
    rw [← pow_two]
    rw [mul_assoc]
    rw [← pow_two]
  ...
  rw [one_mul, mul_assoc, mul_left_inv, mul_one] at res
  exact res
